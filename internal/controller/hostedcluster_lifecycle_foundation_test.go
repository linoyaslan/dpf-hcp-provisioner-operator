...
var _ = Describe("HostedCluster Lifecycle - Foundation & Secret Management", func() {
	...
	BeforeEach(func() {
		...
		sshKey := &corev1.Secret{
			...
			Data: map[string][]byte{
				...
				"ssh-publickey":  []byte("ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQ..."),
			},
		}
	...
	Context("Secret Copying to Clusters Namespace", func() {
		...
		XIt("should copy ssh-key with correct type and labels", func() {
			...
			// Verify data is copied
			Expect(sshKeyTarget.Data).To(HaveKey("ssh-publickey"))
		})
	})
	...
	Context("Phase Computation", func() {
		// TODO: Fix phase computation - verify controller sets Pending correctly
		XIt("should set phase to Pending after successful secret creation", func() {
			// Create DPFHCPBridge
			bridge := &provisioningv1alpha1.DPFHCPBridge{
				ObjectMeta: metav1.ObjectMeta{
					Name:      bridgeName,
		...
		// TODO: Fix phase computation - controller sets Failed instead of Deleting
		XIt("should set phase to Deleting when CR is being deleted", func() {
			// Create DPFHCPBridge
			bridge := &provisioningv1alpha1.DPFHCPBridge{
				ObjectMeta: metav1.ObjectMeta{
					Name:      bridgeName,
