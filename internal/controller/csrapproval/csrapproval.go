...
func (a *CSRApprover) processPendingCSRs(ctx context.Context, dpfhcp *provisioningv1alpha1.DPFHCPProvisioner, hcClient *kubernetes.Clientset) error {
	...
	for _, csr := range pendingCSRs {
		if csr.Spec.SignerName == SignerBootstrap {
			bootstrapCSRs = append(bootstrapCSRs, csr)
		} else if csr.Spec.SignerName == SignerServing {
	...
	for _, csr := range servingCSRs {
		_, err := a.processCSR(ctx, dpfhcp, hcClient, &csr, "serving")
		if err != nil {
			// Actual error - log and continue
...
// Returns:
// - (true, nil): CSR was successfully approved
// - (false, nil): CSR was skipped (validation failed, but not an error condition)
// - (false, error): Actual error occurred during processing
func (a *CSRApprover) processCSR(ctx context.Context, dpfhcp *provisioningv1alpha1.DPFHCPProvisioner, hcClient *kubernetes.Clientset, csr *certv1.CertificateSigningRequest, csrType string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to validate CSR ownership: %w", err)
	}
	if !result.Valid {
